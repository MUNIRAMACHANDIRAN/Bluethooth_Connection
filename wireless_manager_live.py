...
class WirelessManagerApp:
    ...
    def update_ble_list(self, devices):
        ...
        for d in devices:
            ...
            rssi = getattr(d, "rssi", d.metadata.get("rssi", 0))
            self.ble_listbox.insert(END, f"{name} — {d.address} (RSSI: {rssi})")
    ...
    def ble_save_report(self):
        ...
        data = [{"name": d.name,"address":d.address,"rssi":getattr(d, "rssi", d.metadata.get("rssi",0))} for d in self.ble_devices.values()]
    ...
    def wifi_connect_selected(self):
        ...
        if not sel:
            ...
            return
        ssid = list(self.wifi_networks.keys())[sel[0]]
        password = None
        auth = self.wifi_networks[ssid].get("Auth","")
        if "WPA" in auth or "WEP" in auth:
            password = simple_input("Enter Wi-Fi password", f"Password for {ssid}:")
            if not password:
                return
...
"""
    ...
    def wifi_save_report(self):
        ...
        with open(fname,"w") as f:
            json.dump(list(self.wifi_networks.values()),f,indent=2)
